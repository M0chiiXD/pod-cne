package funkin.backend;

// currently just taken from Psych ill recode this shit later ok? ok -myra
import flixel.math.FlxRect;
import flixel.FlxSprite;
import flixel.math.FlxPoint;
import flixel.math.FlxMath;
import Std;

class FunkinBar extends flixel.FlxObject
{
	public var leftBar:FlxSprite;
	public var rightBar:FlxSprite;
	public var bg:FlxSprite;
	public var valueFunction:Void->Float = null;
	public var percent:Float = 0;
	public var bounds:Dynamic = {min: 0, max: 1};
	public var leftToRight:Bool = true;
	public var barCenter:Float = 0;

	// you might need to change this if you want to use a custom bar
	public var barWidth:Int = 1;
	public var barHeight:Int = 1;
	public var barOffset:FlxPoint;

	public var children:Array<FlxSprite> = [];

	public function new(x:Float, y:Float, w:Float, h:Float, image:String = 'game/healthBar', valueFunction:Void->Float = null, boundX:Float = 0, boundY:Float = 1)
	{
		barOffset = FlxPoint.get(3, 3);
		super();
		setPosition(x, y);
		
		this.valueFunction = valueFunction;
		setBounds(boundX, boundY);
		
		bg = new FlxSprite().loadGraphic(Paths.image(image));
		bg.antialiasing = true;
		barWidth = Std.int(bg.width - 6);
		barHeight = Std.int(bg.height - 6);

		leftBar = new FlxSprite().makeGraphic(Std.int(bg.width), Std.int(bg.height), -1);
		leftBar.antialiasing = antialiasing = true;

		rightBar = new FlxSprite().makeGraphic(Std.int(bg.width), Std.int(bg.height), -1);
		rightBar.color = 0x000000;
		rightBar.antialiasing = true;

		regenerateClips();

		children = [bg, rightBar, leftBar];
	}

	public function addFuckers(husband, index) {
		for (i in children) {
			husband.insert(index, i);
			i.camera = camera;
		}
	}

	public function forEach(func) {
		for (i in children) {
			func(i);
		}
	}

	public var enabled:Bool = true;
	public override function update(elapsed:Float) {
		bg.setPosition(x, y);

		if(!enabled)
		{
			super.update(elapsed);
			return;
		}

		if(valueFunction != null)
		{
			var value:Null<Float> = FlxMath.remapToRange(FlxMath.bound(valueFunction(), bounds.min, bounds.max), bounds.min, bounds.max, 0, 100);
			set_percent(value != null ? value : 0);
		}
		else set_percent(0);
		super.update(elapsed);
	}
	
	public function setBounds(min:Float, max:Float)
	{
		bounds.min = min;
		bounds.max = max;
	}

	public function setColors(left:Int = null, right:Int = null)
	{
		if (left != null)
			leftBar.color = left;
		if (right != null)
			rightBar.color = right;
	}

	public function updateBar()
	{
		if(leftBar == null || rightBar == null) return;

		leftBar.setPosition(bg.x, bg.y);
		rightBar.setPosition(bg.x, bg.y);

		var leftSize:Float = 0;
		if(leftToRight) leftSize = FlxMath.lerp(0, barWidth, percent / 100);
		else leftSize = FlxMath.lerp(0, barWidth, 1 - percent / 100);

		leftBar.clipRect.width = leftSize;
		leftBar.clipRect.height = barHeight;
		leftBar.clipRect.x = barOffset.x;
		leftBar.clipRect.y = barOffset.y;

		rightBar.clipRect.width = barWidth - leftSize;
		rightBar.clipRect.height = barHeight;
		rightBar.clipRect.x = barOffset.x + leftSize;
		rightBar.clipRect.y = barOffset.y;

		barCenter = leftBar.x + leftSize + barOffset.x;

		// flixel is retarded
		leftBar.clipRect = leftBar.clipRect;
		rightBar.clipRect = rightBar.clipRect;
	}

	public function regenerateClips()
	{
		if(leftBar != null)
		{
			leftBar.setGraphicSize(Std.int(bg.width), Std.int(bg.height));
			leftBar.updateHitbox();
			leftBar.clipRect = new FlxRect(0, 0, Std.int(bg.width), Std.int(bg.height));
		}
		if(rightBar != null)
		{
			rightBar.setGraphicSize(Std.int(bg.width), Std.int(bg.height));
			rightBar.updateHitbox();
			rightBar.clipRect = new FlxRect(0, 0, Std.int(bg.width), Std.int(bg.height));
		}
		updateBar();
	}

	public function set_percent(value:Float)
	{
		var doUpdate:Bool = false;
		if(value != percent) doUpdate = true;
		percent = value;

		if(doUpdate) updateBar();
		return value;
	}

	public function set_leftToRight(value:Bool)
	{
		var old = leftToRight;
		leftToRight = value;
		if (value != old) updateBar();
		return value;
	}

	public function set_barWidth(value:Int)
	{
		var old = barWidth;
		barWidth = value;
		if (value != old) regenerateClips();
		return value;
	}

	public function set_barHeight(value:Int)
	{
		var old = barHeight;
		barHeight = value;
		if (value != old) regenerateClips();
		return value;
	}
}